...
const { query: dbQuery, colunaExiste } = require('./db');
...
function permitirContactosSemMarca(q = dbQuery) {
  if (!semMarcaGarantido) {
    semMarcaGarantido = (async () => {
      // Depois da 051 a coluna já não existe e o ALTER só servia para deixar
      // um erro 42703 nos logs da base de dados.
      if (!await colunaExiste('contacts', 'brand_id', q)) return;
      await q('ALTER TABLE contacts ALTER COLUMN brand_id DROP NOT NULL')
        .catch(() => { /* sem privilégio, ou apagada entretanto */ });
    })();
  }
...
function garantirColunaOculto(q = dbQuery) {
  if (!colunaOculto) {
    colunaOculto = (async () => {
      // Verificar primeiro, e só correr o ALTER se a coluna faltar mesmo. O
      // ALTER TABLE — mesmo ADD COLUMN IF NOT EXISTS, mesmo quando a coluna já
      // existe — pega um ACCESS EXCLUSIVE lock na tabela contacts. Corria na
      // ligação da transacção de importação (via marcarOculto dentro do
      // upsertContactos), por isso, com dois blocos a importar em paralelo, o
      // lock de um chocava com o INSERT do outro e a transacção do bloco
      // rebentava — os contactos desse bloco voltavam como "falharam a gravar".
      // Uma consulta ao catálogo é barata e não pega lock nenhum; depois de a
      // coluna existir, o ALTER nunca mais corre. (Mesmo padrão da
      // permitirContactosSemMarca.)
      try {
        if (await colunaExiste('contacts', 'hidden', q)) return;
      } catch (_) { /* sem catálogo acessível: tenta o ALTER à mesma, em baixo */ }
      await q('ALTER TABLE contacts ADD COLUMN IF NOT EXISTS hidden BOOLEAN NOT NULL DEFAULT FALSE')
...
async function upsertContactos(q, linhas, opcoes = {}) {
  ...
  const novosIds = resultado.filter(r => r.criado && r.id).map(r => r.id);

  // G-3: registar a evidência de consentimento nos contactos criados agora —
  // origem e data, tiradas do que já se sabe (source/created_at). A base legal
  // (legal_basis) fica NULL: é decisão jurídica, não técnica. COALESCE para
  // nunca reescrever uma evidência já registada.
  //
  // A coluna só existe depois de a migração 059 correr — mas NÃO se pode
  // apanhar o 42703 aqui e continuar: um erro dentro de uma transacção aborta
  // a transacção inteira no Postgres, e a próxima instrução (marcarOculto, o
  // COMMIT) rebenta com "current transaction is aborted". Era isto que fazia
  // perder todos os blocos com contactos novos. Verifica-se antes, com uma
  // consulta ao catálogo, que não suja a transacção.
  if (novosIds.length && await colunaExiste('contacts', 'consent_source', q)) {
    await q(
      `UPDATE contacts
  ...
  if (ocultarNovos) {
    await marcarOculto(q, novosIds, true);
  }
  return resultado;
}
