 ...
 */
async function runBatch(campaignId, actorId = null, options = {}) {
  ...
  // Modo simulação (dry-run): processa tudo (deduplicação, variáveis, tracking)
  // mas não chama o SES. Útil para testar performance e lógica com listas grandes.
  const DRY_RUN = process.env.SEND_DRY_RUN === 'true' || options?.dryRun === true;
  ...
  // Dry-run: skip quota check — sem SES não há quota a consumir
  ...
  for (let i = 0; i < toSend.length; i += RATE) {
    ...
    await Promise.all(toSend.slice(i, i + RATE).map(async contact => {
      ...
      try {
        ...
        let msgId = null;
        if (DRY_RUN) {
          // Simulação: todo o processamento foi feito (variáveis, tracking, HTML),
          // mas não se chama o SES. Message ID fictício para manter consistência na BD.
          msgId = `dryrun-${campaignId}-${contact.contact_id}-${Date.now()}`;
        } else {
  ...
  return { done: remaining === 0, sent, failed, remaining, ...(DRY_RUN ? { dryRun: true } : {}) };
}
