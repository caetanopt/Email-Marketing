...
// Sends a completion summary email to addresses configured in global_settings.notification_emails
async function sendCampaignCompletionNotification({ campaignName, brandName, totalSent, totalFailed }) {
  try {
    let emails = [];
    try {
      const rows = await query('SELECT notification_emails FROM global_settings WHERE id=1');
      const raw = rows[0]?.notification_emails || '';
      emails = raw.split(',').map(e => e.trim()).filter(e => e.includes('@'));
    } catch (_) { return; } // table or column may not exist yet
    ...
    const failLine = totalFailed > 0
      ? `<tr><td style="padding:4px 0;color:#64748b;font-size:13px">Falhas</td><td style="padding:4px 0;font-size:13px;font-weight:600;color:#dc2626">${totalFailed.toLocaleString('pt-PT')}</td></tr>`
      : '';
    const htmlBody = `<!DOCTYPE html><html><body style="margin:0;padding:0;background:#f8fafc;font-family:Arial,sans-serif">
...
<table width="520" cellpadding="0" cellspacing="0" style="background:#ffffff;border-radius:12px;border:1px solid #e2e8f0;overflow:hidden">
  <tr><td style="background:#0f172a;padding:20px 32px">
    <p style="margin:0;color:#fff;font-size:16px;font-weight:700">PrimeMail</p>
  </td></tr>
  <tr><td style="padding:32px">
    <p style="margin:0 0 4px;font-size:11px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:#94a3b8">ENVIO CONCLUÍDO</p>
    <h1 style="margin:0 0 24px;font-size:20px;color:#0f172a">${campaignName}</h1>
    <table cellpadding="0" cellspacing="0" style="width:100%;border-top:1px solid #e2e8f0">
      <tr><td style="padding:12px 0;color:#64748b;font-size:13px">Marca</td><td style="padding:12px 0;font-size:13px;font-weight:600;color:#0f172a">${brandName}</td></tr>
      <tr style="background:#f0fdf4"><td style="padding:12px 8px;color:#64748b;font-size:13px;border-radius:6px 0 0 6px">Enviados com sucesso</td><td style="padding:12px 8px;font-size:13px;font-weight:700;color:#16a34a;border-radius:0 6px 6px 0">${totalSent.toLocaleString('pt-PT')}</td></tr>
      ${failLine}
      <tr><td style="padding:12px 0;color:#64748b;font-size:13px">Total</td><td style="padding:12px 0;font-size:13px;font-weight:600;color:#0f172a">${(totalSent + totalFailed).toLocaleString('pt-PT')}</td></tr>
      <tr><td style="padding:12px 0;color:#64748b;font-size:13px">Data/hora</td><td style="padding:12px 0;font-size:13px;color:#0f172a">${nowPt}</td></tr>
    </table>
    <p style="margin:24px 0 0;text-align:center">
      <a href="${APP_URL}" style="display:inline-block;padding:10px 24px;background:#0f172a;color:#fff;border-radius:8px;text-decoration:none;font-size:13px;font-weight:600">Abrir PrimeMail</a>
    </p>
  </td></tr>
</table>
</td></tr>
    ...
    const textBody = `Campanha enviada: ${campaignName}\nMarca: ${brandName}\nEnviados: ${totalSent}\nFalhas: ${totalFailed}\nData: ${nowPt}\n\n${APP_URL}`;
...
module.exports = { initCampaignSend, runBatch, sendCampaignCompletionNotification };
