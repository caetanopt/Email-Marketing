...
//
// Por agora só uma: desligar a frase legal do rodapé (o remetente, o ano e a
// sede da empresa). Vive num módulo próprio porque tem de ser lida da mesma
// maneira nos quatro caminhos que montam o rodapé — envio real, envio de
// teste, pré-visualização no ecrã e versão web — e escrita nos dois que
// gravam a campanha.
...
function garantirColunaFraseLegal(q = dbQuery) {
  if (!colunaGarantida) {
...
// Uma campanha sem a coluna (ou com ela a false) leva a frase legal: o
// comportamento por omissão é sempre enviá-la.
function semFraseLegal(campanha) {
...
async function gravarFraseLegal(id, valor, q = dbQuery) {
  if (valor === undefined || valor === null) return;
  await garantirColunaFraseLegal(q);
  try {
...
// false, que é enviar a frase.
async function lerFraseLegal(id, q = dbQuery) {
  try {
