...
async function bloquearCancelados(campaignId, q = dbQuery) {
  // A barreira cobria só 'pending' e 'retry'. Desde a migração 060 existe
  // 'sending', e uma linha que fique órfã nesse estado — função morta a meio,
  // browser fechado — atravessava esta barreira em todas as chamadas
  // seguintes e era enviada quando fosse recuperada, a quem entretanto tinha
  // cancelado. Em matéria de subscrição isso é pior do que um duplicado.
  //
  // Mas nem todas as linhas em 'sending' são órfãs: a maioria pertence a um
  // lote a decorrer AGORA, possivelmente noutro worker. Marcar essas como
  // falhadas seria corromper o estado de quem está a trabalhar. Por isso o
  // alcance é estreito e igual ao da recuperação de órfãs no motor:
  //   • reclamada há mais de 2 minutos (nenhuma invocação dura tanto), e
  //   • sem prova de entrega — o que já saiu para o SES não se marca como
  //     falhado, porque saiu mesmo.
  const ORFA = `(status = 'sending' AND attempted_at < NOW() - INTERVAL '2 minutes' AND dispatched_at IS NULL)`;
  const ORFA_SEM_PROVA = `(status = 'sending' AND attempted_at < NOW() - INTERVAL '2 minutes')`;
  const onde = (orfa) => `WHERE campaign_id=$1
         AND (status IN ('pending','retry') OR ${orfa})
         AND (${SUPRIMIDO} OR ${CANCELADO})`;

  const executar = async (orfa) => q(
  ...
  // Pergunta-se ao catálogo em vez de tentar e apanhar o erro: um 42703 fica
  // registado nos logs do Postgres a cada lote, e dentro de uma transacção
  // aborta-a (foi assim que a importação partiu uma vez).
  const temProva = await colunaExiste('campaign_recipients', 'dispatched_at', q);
  const orfa = temProva ? ORFA : ORFA_SEM_PROVA;
  try {
    return await executar(orfa);
  } catch (e) {
    // 42703: base de dados antiga, sem attempted_at/error_message.
    if (e.code !== '42703') throw e;
    return q(`UPDATE campaign_recipients SET status='failed' ${onde(orfa)}`, [campaignId]);
  }
}
