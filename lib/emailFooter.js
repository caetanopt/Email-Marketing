...
// As cinco redes do rodapé passaram a ter ícone próprio, servido do nosso
// domínio a partir de /social/<rede>.png (ficheiros estáticos — ver
// social/LEIA-ME.md; não é uma rota de API porque o limite de funções do plano
// já estava esgotado). Duas razões:
//
//  - o conjunto da MJML não tem TikTok. A lista de redes aqui abaixo já o
//    oferecia, portanto quem o preenchesse sem indicar um ícone ficava com um
//    quadrado partido no rodapé de todos os emails;
//  - os restantes estão datados (identidades antigas do Facebook e do
//    Instagram) e são claros, feitos para fundos escuros — daí a nota acima
//    sobre desaparecerem no #f1f1f1 do rodapé. Os novos têm a cor da marca em
//    fundo cheio, por isso vêem-se em qualquer fundo.
const SOCIAL_ICON_BASE = (process.env.APP_URL || 'https://emkt.caetano.pt').replace(/\/$/, '');
const SOCIAL_ICON_PROPRIOS = ['facebook', 'instagram', 'x', 'youtube', 'linkedin', 'tiktok'];
const SOCIAL_ICON_DEFAULT = (network) =>
  SOCIAL_ICON_PROPRIOS.includes(network)
    ? `${SOCIAL_ICON_BASE}/social/${network}.png`
 ...
 * @param {boolean} [o.semRodapeLegal]  omite o bloco legal por baixo da área
 *                                      cinzenta: a frase (remetente e sede) e a
 *                                      linha de links. O cabeçalho
 *                                      List-Unsubscribe não é afectado — é ele
 *                                      que dá o botão de cancelamento do Gmail.
 */
function buildLegalFooter(o = {}) {
  ...
  // Todo o bloco legal — a frase e a linha de links — pode ser desligado por
  // campanha, no ecrã de envio. O cabeçalho List-Unsubscribe continua a ser
  // enviado (ver lib/rawEmail.js): é ele que dá o botão de cancelamento do
  // Gmail e do Yahoo, e é o que essas regras de envio em volume exigem.
  const semBloco = o.semRodapeLegal === true;
  const frase = semBloco ? '' :
    `Este e-mail foi enviado para ${emailTxt ? `<a href="mailto:${emailTxt}" style="color:#8a8a8a;text-decoration:underline">${emailTxt}</a>` : '—'} por © ${ano} Caetano Automotive Portugal, S.A., com sede na Rua do Barreiro 547, 4409-513 Vila Nova de Gaia, matriculada na CRCOM.VNGAIA com o Nº/NIPC 500003165.`;

  const link = (href, texto) =>
    `<a href="${escHtml(href)}" target="_blank" style="color:#8a8a8a;text-decoration:underline">${texto}</a>`;
  const links = semBloco ? '' : [
  ...
  // Sem área cinzenta e sem bloco legal não há rodapé nenhum: devolve-se vazio
  // em vez de uma tabela oca, que só acrescentava altura ao fim do email.
  if (!areaCinza && !frase && !links) return '';
  ...
  <tr><td align="center" style="padding:0">
    <table role="presentation" width="${largura}" cellpadding="0" cellspacing="0" border="0" style="max-width:${largura}px;width:100%">
      ${areaCinza}
      ${frase ? `<tr><td align="center" style="padding:18px 28px 6px;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.2;color:#8a8a8a">${frase}</td></tr>` : ''}
      ${links ? `<tr><td align="center" style="padding:${frase ? '0' : '18px'} 28px 24px;font-family:Arial,Helvetica,sans-serif;font-size:12px;line-height:1.2;color:#8a8a8a">${links}</td></tr>` : ''}
    </table>
  </td></tr>
</table>`;
}
