...
// Show preview pane alongside the visual builder. Persists between sessions.
const showPreview = ref(localStorage.getItem('tpl_show_preview') !== 'false');
watch(showPreview, (v) => localStorage.setItem('tpl_show_preview', v ? 'true' : 'false'));
        ...
        <form @submit.prevent="submit" class="h-full">
            <div class="flex flex-col gap-4 h-[calc(100vh-120px)]">
                ...
                <div class="bg-white border border-slate-200 rounded-xl p-4 flex flex-wrap items-end gap-4">
                    ...
                    <!-- Toggle preview pane (only visible in visual mode) -->
                    <button v-if="mode === 'visual'" type="button" @click="showPreview = !showPreview"
                            :class="[
                ...
                <div class="flex-1 min-h-0">
                    ...
                    <!-- Visual builder + live iframe preview -->
                    <div v-show="mode === 'visual'"
                         :class="[
                             'h-full grid gap-4',
                             showPreview ? 'grid-cols-[minmax(0,1fr)_minmax(360px,420px)]' : 'grid-cols-1'
                         ]">
                        ...
                        <div v-if="showPreview"
                             class="bg-white border border-slate-200 rounded-xl overflow-hidden flex flex-col min-w-0">
                            <div class="px-4 py-2.5 border-b border-slate-200 bg-slate-50 flex items-center gap-2">
                                <div class="w-2.5 h-2.5 rounded-full bg-red-400"></div>
                                <div class="w-2.5 h-2.5 rounded-full bg-yellow-400"></div>
                                <div class="w-2.5 h-2.5 rounded-full bg-green-400"></div>
                                <span class="ml-2 text-xs text-slate-500 font-mono">preview</span>
                                <span v-if="isCompiling" class="ml-auto text-xs text-slate-400 flex items-center gap-1">
                                ...
                                <span v-else-if="previewError" class="ml-auto text-xs text-red-500">Erro</span>
                                <span v-else-if="previewHtml" class="ml-auto text-xs text-green-600">Compilado</span>
                            </div>
                            <div class="flex-1 overflow-hidden bg-slate-100">
                                <iframe v-if="previewHtml"
                                        :srcdoc="previewHtml"
                                ...
                                <div v-else class="flex items-center justify-center h-full text-slate-400 text-xs px-4 text-center">A compilar preview…</div>
                            </div>
                        </div>
                    </div>
