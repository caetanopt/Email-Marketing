...
// Tira caracteres de controlo dos campos de texto vindos do ficheiro. O byte
// nulo (0x00) e outros controlos C0 aparecem em exports de Excel e de bases
// de dados, e o Postgres recusa-os no texto — uma única linha assim rebentava
// a transacção do bloco inteiro e levava à frente ~500 contactos bons. Mantém
// tab, newline e carriage return; remove o resto do intervalo de controlo.
function semControlo(v) {
  if (v == null) return v;
...
async function processBatch(listId, batch, { ocultarNovos = false } = {}) {
  ...
  for (const c of batch) {
    ...
    validRows.push({ ...c, email, name: semControlo(c.name), phone: semControlo(c.phone), company: semControlo(c.company) });
  }
  ...
  if (validRows.length) {
    ...
    // tranca a tabela, e dentro da transacção ficaria trancada até ao fim do
    // lote. O mesmo vale para a coluna 'hidden' (garantirColunaOculto), que o
    // upsertContactos toca lá dentro via marcarOculto quando ocultarNovos.
    // Corre uma vez, fora da transacção; depois de a coluna existir, nunca
    // mais corre.
    await permitirContactosSemMarca();
    if (ocultarNovos) await garantirColunaOculto();

    // Grava um conjunto de linhas numa só transacção. Devolve quantas ficaram
    // e quais não voltaram com id. Não mexe nos contadores: quem chama é que
    // os aplica, e só depois do commit — para uma transacção que rebenta não
    // deixar contagens a mais.
    const gravarLote = (linhas) => transaction(async q => {
      const todos = await upsertContactos(q, linhas.map(c => ({
        email: c.email,
        name: c.name || null,
        phone: c.phone || null,
      ...
      const naoGravados = todos.filter(r => !r.id).map(r => r.email);

      if (listId && inserted.length) {
        // added_at leva a mesma data do ficheiro, quando existe.
        const dataPorEmail = new Map(linhas.map(c => [c.email, normalizeSubscribedAt(c.subscribed_at)]));
        ...
        const extraPorEmail = new Map(linhas.filter(c => c._extra_data).map(c => [c.email, c._extra_data]));
        for (const [email, extra] of extraPorEmail) {
          const cid = emailToId[email];
          if (!cid) continue;
          await q(
            'UPDATE list_members SET extra_data=$1::jsonb WHERE list_id=$2 AND contact_id=$3',
            [JSON.stringify(extra), listId, cid]
          );
        }
      }
      return { insertedCount: inserted.length, naoGravados };
    });

    // A transacção é toda ou nada: uma única linha com dados que o Postgres
    // recusa (um byte de controlo que escapou, uma data impossível, o que
    // for) rebentava-a e levava à frente ~500 contactos bons. Em vez de dar
    // o bloco inteiro como falhado, biseca-se: grava-se metade de cada vez,
    // e só a linha que chega sozinha e ainda rebenta é dada como falhada,
    // com o motivo. O caminho normal (bloco que grava à primeira) não paga
    // nada disto.
    const gravarComBiseccao = async (linhas) => {
      if (!linhas.length) return;
      try {
        const { insertedCount, naoGravados } = await gravarLote(linhas);
        imported += insertedCount;
        naoGravados.forEach(email => { skipped++; skipped_detail.push({ email, reason: 'nao_gravado' }); });
      } catch (err) {
        if (linhas.length === 1) {
          failed++;
          const motivo = String(err && err.message || err || '').slice(0, 200);
          skipped_detail.push({ email: linhas[0].email, reason: 'falha_ao_gravar', detalhe: motivo });
          console.error('processBatch: linha rejeitada', linhas[0].email, motivo);
          return;
        }
        console.warn(`processBatch: bloco de ${linhas.length} rebentou, a bisecar (${err && err.message})`);
        const meio = Math.floor(linhas.length / 2);
        await gravarComBiseccao(linhas.slice(0, meio));
        await gravarComBiseccao(linhas.slice(meio));
      }
    };
    await gravarComBiseccao(validRows);
  }
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    if (action === 'retention_cleanup' && req.method === 'POST') {
      ...
      try {
        ...
        const apagados = await transaction(async (q) => {
          ...
          // Catálogo, não try/catch: um 42P01 apanhado aqui deixaria a
          // transacção abortada e o DELETE a seguir rebentava.
    ...
    if (req.method === 'DELETE') {
      ...
      const apagados = await transaction(async (q) => {
        // Catálogo, não try/catch: um 42P01 apanhado aqui abortava a transacção
        // e o DELETE a seguir rebentava.
        if (await colunaExiste('email_send_log', 'email', q)) {
